...
COL_NAMES = ["Accel X (g)", "Accel Y (g)", "Accel Z (g)",
...
FALL_LABELS = {
    ...
    "F04": "Forward trip", "F05": "Jogging trip",  "F06": "Fainting (vert)",
    "F07": "Fainting (table)", "F08": "Getting up fwd", "F09": "Getting up lat",
    "F10": "Sitting fwd", "F11": "Sitting bwd", "F12": "Sitting lat",
    "F13": "Seated faint fwd", "F14": "Seated faint bwd", "F15": "Seated faint lat",
}
ADL_LABELS = {
    "D01": "Walk slow", "D02": "Walk fast", "D03": "Jog slow", "D04": "Jog fast",
    "D05": "Stairs slow", "D06": "Stairs fast", "D07": "Sit half slow",
    "D08": "Sit half fast", "D09": "Sit low slow", "D10": "Sit low fast",
    "D11": "Collapse to chair", "D12": "Lie slow", "D13": "Lie fast",
    "D14": "Lateral roll", "D15": "Kneel bend", "D16": "Bend no knees",
    "D17": "Get in car", "D18": "Stumble", "D19": "Jump",
}
...
def plot_signal_lengths(records: list[dict], out: Path) -> None:
    ...
    ax.hist(falls, bins=bins, alpha=0.7, label=f"Fall (n={len(falls)})", color="#e74c3c")
...
def plot_sample_waveforms(records: list[dict], out: Path) -> None:
    ...
    fig.suptitle("Sample Waveforms — Fall (F06 fainting) vs ADL (D02 fast walk)",
                 fontsize=13, fontweight="bold")

    titles = [f"{fall_ex['code']} — {FALL_LABELS[fall_ex['code']]} "
...
def plot_accel_magnitude(records: list[dict], out: Path) -> None:
    ...
    parts = ax.violinplot([adl_peaks, fall_peaks], positions=[1, 2],
    ...
    ax.annotate(f"median={med_adl:.2f}g",  xy=(1, med_adl),  xytext=(1.15, med_adl + 0.5),  fontsize=9)
    ax.annotate(f"median={med_fall:.2f}g", xy=(2, med_fall), xytext=(2.05, med_fall + 0.5), fontsize=9)
...
def print_report(records: list[dict]) -> None:
    ...
    # Subject breakdown
    subj_types = Counter("SA" if r["subject"].startswith("SA") else "SE"
                         for r in records)
    print()
    print(f"  Subjects: SA (young adults) = {subj_types['SA']} files, "
