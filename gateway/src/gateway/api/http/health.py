...
@router.get(
    ...
    description=(
        "Unconditional 200 once the process can respond at all. "
        "This is the one contract every deployment surface depends on: "
        "Docker healthcheck, Kubernetes liveness probe, and the container's "
        "`HEALTHCHECK` instruction.\n\n"
        "It must **never** depend on anything that could itself be degraded — "
        "use `/ready` for that."
    ),
)
...
@router.get(
    ...
    description=(
        "Gates traffic behind the background-task startup phase.\n\n"
        "- **503** until `main.py`'s lifespan has finished starting the "
        "background tasks and initializing the event store.\n"
        "- **503** again if the event bus backend is Redis and Redis becomes "
        "unreachable mid-flight.\n"
        "- **200** when the instance is fully ready.\n\n"
        "Kubernetes readiness probe targets this endpoint; the liveness "
        "probe stays on `/health`."
    ),
)
async def ready(request: Request) -> dict[str, str]:
    if not request.app.state.ready:
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE, detail="startup not complete"
        )

    event_bus = request.app.state.container.event_bus
    if isinstance(event_bus, RedisStreamsEventBus) and not await event_bus.ping():
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE, detail="event bus unreachable"
        )
